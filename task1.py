...
def clean_data(df):
    ...
    df = df.where(df["sex_bin"] == "", 1)
    df = df.where(df["species"] == "", 1)
    df = df.where(df["island_bin"] == "", 1)

    df = df.where(df["bill_length_mm"] == "", 1)
    df = df.where(df["bill_depth_mm"] == "", 1)
    df = df.where(df["flipper_length_mm"] == "", 1)
...
if __name__ == '__main__':
    ...
    x_penguins, y_penguins = clean_data(penguins)
